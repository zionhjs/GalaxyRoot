package com.galaxy.common.core.service;

import com.galaxy.common.core.Mapper.Mapper;
import com.galaxy.common.core.exception.BusinessException;
import com.galaxy.common.core.vo.Page;
import org.apache.commons.lang3.StringUtils;
import org.apache.ibatis.exceptions.TooManyResultsException;
import org.springframework.beans.factory.annotation.Autowired;
import tk.mybatis.mapper.entity.Condition;
import tk.mybatis.mapper.entity.Example;

import java.lang.reflect.Field;
import java.lang.reflect.ParameterizedType;
import java.util.Date;
import java.util.List;
import java.util.Map;

/**
 * 基于通用MyBatis Mapper插件的Service接口的实现
 */
public class AbstractService<T> implements Service<T>  {

    @Autowired
    protected Mapper<T> mapper;
    // 当前泛型真实类型的Class
    private Class<T> modelClass;

    public AbstractService() {
        ParameterizedType pt = (ParameterizedType) this.getClass().getGenericSuperclass();
        modelClass = (Class<T>) pt.getActualTypeArguments()[0];
    }

    @Override
    public void save(T model) {
        mapper.insertSelective(model);
    }
    @Override
    public int saveList(List<T> model) {
        return mapper.insertList(model);
    }

    @Override
    public void save(List<T> models) {
        mapper.insertList(models);
    }

    @Override
    public void deleteById(Long id) {
        mapper.deleteByPrimaryKey(id);
    }

    @Override
    public void changeStateById(Long id, Boolean isDelete,Long userId) {
        try {
            T model = modelClass.newInstance();
            Field field = modelClass.getDeclaredField("isDelete");
            field.setAccessible(true);
            field.set(model, isDelete);
            field = modelClass.getDeclaredField("id");
            field.setAccessible(true);
            field.set(model, id);
            field = modelClass.getDeclaredField("updateUserId");
            field.setAccessible(true);
            field.set(model, userId);
            field = modelClass.getDeclaredField("updateTime");
            field.setAccessible(true);
            field.set(model, new Date());
            mapper.updateByPrimaryKeySelective(model);
        } catch (ReflectiveOperationException e) {
            e.printStackTrace();
        }

    }

    @Override
    public void deleteByIds(String ids) {
        mapper.deleteByIds(ids);
    }

    @Override
    public void update(T model) {
        mapper.updateByPrimaryKeySelective(model);
    }

    @Override
    public T findById(Long id) {
        return mapper.selectByPrimaryKey(id);
    }

    @Override
    public T findDetail(Long id) {
        try {
            T model = modelClass.newInstance();
            Field field = modelClass.getDeclaredField("id");
            field.setAccessible(true);
            field.set(model, id);
            field = modelClass.getDeclaredField("isDelete");
            field.setAccessible(true);
            field.set(model, false);
            return mapper.selectOne(model);
        } catch (InstantiationException e) {
            e.printStackTrace();
        } catch (IllegalAccessException e) {
            e.printStackTrace();
        } catch (NoSuchFieldException e) {
            e.printStackTrace();
        }
        return null;
    }

    @Override
    public T findBy(String fieldName, Object value) throws TooManyResultsException {
        try {
            T model = modelClass.newInstance();
            Field field = modelClass.getDeclaredField(fieldName);
            field.setAccessible(true);
            field.set(model, value);
            return mapper.selectOne(model);
        } catch (ReflectiveOperationException e) {
            throw new BusinessException(e.getMessage(), e);
        }
    }

    @Override
    public T findBy(T model) throws TooManyResultsException {
        return mapper.selectOne(model);
    }

    @Override
    public List<T> findByModel(T model) throws TooManyResultsException {
        return mapper.select(model);
    }
    @Override
    public List<T> findValidAll() throws TooManyResultsException {
        try {
            T model = modelClass.newInstance();
            Field field = modelClass.getDeclaredField("state");
            field.setAccessible(true);
            field.set(model, "1");
            return mapper.select(model);
        } catch (ReflectiveOperationException e) {
            throw new BusinessException(e.getMessage(), e);
        }
    }

    @Override
    public List<T> findValidDeleteAll(Page<T> page) {
        try {
            T model = page.getModel();
            Condition condition = new Condition(modelClass);
            Example.Criteria criteria = condition.createCriteria();
            if (model != null) {
                Field[] fields = modelClass.getDeclaredFields();
                for (Field field : fields) {
                    field.setAccessible(true);
                    if ("isDelete".equals(field.getName())) {
                        continue;
                    }
                    if(field.get(model)!=null){
                        if (field.getType() == String.class){
                            String fieldValue = (String) field.get(model);
                            if (StringUtils.isNotBlank(fieldValue)) {
                                if (fieldValue.contains("%")) {
                                    criteria.andLike(field.getName(), fieldValue);
                                } else {
                                    criteria.andEqualTo(field.getName(), fieldValue);
                                }
                            }
                        }else{
                            criteria.andEqualTo(field.getName(), field.get(model));
                        }
                    }
                }
            }
            Map<String, String> params = page.getParams();
            if(params!=null){
                for (String key : params.keySet()) {
                    if("orderBy".equals(key)){
                        condition.setOrderByClause(params.get(key));
                    }else if(key.contains("or ")) {
                        criteria.orCondition(key.replace("or ",""), params.get(key));
                    }else{
                        criteria.andCondition(key, params.get(key));
                    }
                }
            }

            criteria.andEqualTo("isDelete", false);
            return mapper.selectByCondition(condition);
        } catch (ReflectiveOperationException e) {
            throw new BusinessException(e.getMessage(), e);
        }
    }

    @Override
    public List<T> findByIds(String ids) {
        return mapper.selectByIds(ids);
    }

    @Override
    public List<T> findByCondition(Condition condition) {
        return mapper.selectByCondition(condition);
    }

    @Override
    public List<T> findAll() {
        return mapper.selectAll();
    }

}
